...
- We intentionally use package cmd in /cmd for Cobra command wiring (standard convention)
...
- In library-ish code (helpers), return wrapped errors; don’t log.
- In CLI entrypoints/command execution, log (possibly fatal) and exit; don’t return upward.
